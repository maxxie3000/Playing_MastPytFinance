...
class TickData(object):
    ...
    def __init__(self, timestamp='', symbol='', open_price=0, close_price=0,
                total_volume=0):
            self.symbol = symbol
...
# Class to fetch historical data from an external data provider
class MarketDataSource(object):
    def __init__(self, symbol, tick_event_handler=None, start='',
                end=''):
    ...
    def fetch_historical_prices(self):
        ...
        api_key = 'DxswD4b7N-71v3B7DaXk'
        quandl.ApiConfig.api_key = api_key
        df = quandl.get(self.symbol, start_date=self.start,
                        end_date=self.end)
        return df
    ...
    def run(self):
        ...
        print(f'Processing total_ticks: {total_ticks}')
        ...
        for timestamp, row in self.df.iterrows():
            ...
            print(f'{timestamp.date()} TICK {self.symbol} \n open: {open_price} \n close: {close_price}')
...
# Represents a single order sent by the strategy to the server
class Order(object):
...
# Keep track of our current market position
...
class Strategy:
    ...
    def send_market_order(self, symbol, qty, is_buy, timestamp):
        if self.send_order_event_handler:
            order = Order(timestamp, symbol, qty, is_buy,
                            is_market_order=True, price=0)
...
class MeanRevertingStrategy(Strategy):
    ...
    def generate_signals_and_send_order(self, market_data):
        ...
        if self.buy_threshold > signal_value and not self.is_long:
            print(f'{timestamp.date()} BUY signal')
            ...
            print(f'{timestamp.date()} SELL signal')
            self.send_market_order(
...
class BacktestEngine:
    ...
    def on_order_received(self, order):
        ...
        print(f'{order.timestamp.date()} ORDER', 'BUY' if order.is_buy else 'SELL', \
            f' {order.symbol} {order.qty}'
        )
    ...
    def on_order_filled(self, symbol, qty, is_buy, filled_price, timestamp):
        ...
        print(f'{timestamp.date()} FILLED', 'BUY' if is_buy else 'SELL',
                f' {qty} {symbol} at {filled_price}')
    ...
    def print_position_status(self, market_data):
        for symbol, position in self.positions.items():
            ...
            print(f'{timestamp.date()} POSITION value: {position.position_value:.3f} \
upnl: {upnl:.3f}, rpnl: {position.rpnl:.3f}')
...
for i, (buy_threshold, sell_threshold) in enumerate(thresholds):
    ...
    ax = axes[i // 2, i % 2]
    ax.set_title(f'B/S thresholds:({buy_threshold}, {sell_threshold})')
    df_rpnls.plot(ax=ax)
